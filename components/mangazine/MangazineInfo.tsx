...
export default function MangazineInfo({
  data,
  loading = false,
}: {
  data?: MangazineDetail
  loading?: boolean
}) {
    ...
    <div className="py-3 flex border-b border-gray-200">
      ...
      <div className="m-0 flex-1 text-[#242a36] text-sm font-medium">
        {loading ? (
          ...
          <Link
            target="_blank"
            className="text-[#0189EC] mr-4 inline-block"
            href={link_url}
          >
            {value || '暂无信息'}
          </Link>
  ...
  const icon = <i className="fa-duotone fa-triangle-exclamation"></i>
  return (
    <div className="border-t-2 border-solid border-gray-300 py-6 mt-6">
      ...
      <Alert
        ...
        icon={icon}
      >
        杂志的相关信息均为从Manga Time
        Kirara官网获取，可能会存在一定的误差(如发售时间的波动等)，还请谅解
      </Alert>
      {renderItem({
        label: '名称',
        value: data?.detail.magazine.name || '',
        loading,
      })}
      {renderItem({
        label: '发售时间',
        value: data?.detail.magazine.publish_date || '',
        loading,
      })}
      {renderItem({
        label: '官网链接',
