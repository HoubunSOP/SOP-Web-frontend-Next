...
export default async function Content({id}: { id: number }) {
    ...
    const res = await fetch(`http://127.0.0.1:8000/comics/${id}`, {
        next: {
            revalidate: 60,
            tags: ['collection'],
        },
    ...
    const data = (await res.json()) as ComicDetail
    if (data.status !== 200) {
        redirect('/not-found');
    }
            ...
            <div className="flex flex-col md:flex-row ">
                <div className="info order-last md:order-first md:mr-4">
                    ...
                    <p className="mb-2.5 m-0 text-sm tracking-wide">
                        ...
                        <Link href={"/list/comic?category_id=" + data.detail.categories[0].id} className="inline-block h-7 leading-7 py-0 px-3 bg-[#F2F2F2] rounded-2xl text-[#606060] text-xs">
